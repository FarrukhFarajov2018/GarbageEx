...
namespace GarbageH
{
    ...
    class Action
    {
        ...
        public void sortbyMail(List<Debtor> debtors) //1
        {

            var query =
            from client in debtors
            where client.Email.Contains("rhyta.com") || client.Email.Contains("dayrep.com")
            select client;      
        ...
        public void sortbyAge(List<Debtor> debtors)//2
        {
            var query =
           from client1 in debtors
           where ((DateTime.Now - client1.BirthDay).Days / 365) > 26 && ((DateTime.Now - client1.BirthDay).Days / 365) < 36
            select client1;
        ...
        public void sortbyDebtAmount(List<Debtor> debtors)//3
        {
          
            var query =
         from client1 in debtors
         where client1.Debt<=5000
         select client1;
        ...
        public void sortbySimbolsinNameTelNumber(List<Debtor> debtors)//4
        {
           
            var query =
        from client2 in debtors
        where client2.FullName.Length > 18 || (client2.Phone.Count(y => y == '7') >= 3)
            select client2;
        ...
        public void sortbyBirthMonth(List<Debtor> debtors)//5
        {            
            var query =
       from client in debtors
       where client.BirthDay.Month == 12 || client.BirthDay.Month >= 1 && client.BirthDay.Month <= 2
       select client;
        ...
        public void AverageDebtSort(List<Debtor> debtors)//6
        {
            ...
            int averageDebt = sum / debtors.Count;//average debt amount

            Console.WriteLine($" Average debt is : {averageDebt}");
            Console.WriteLine();

            var query =
    from client4 in debtors
    where client4.Debt > averageDebt 
    orderby client4.FullName.Split(' ').ToList().Last(), 
    client4.Debt
            ...
            void switchFunc()
            {
                while (true)
                {
                    ...
                    Console.WriteLine("6. Sort- Debt more than average debt");
                     Console.WriteLine("7. Press 7 to End Program");
